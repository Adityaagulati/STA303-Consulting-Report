...
model_1 <- glm(promoted ~ productivity + gender , data = testing, family = binomial(link= "logit"))
summary(model_1)
confint(model_1)


#  add leadership for level
model_2 <- glm(promoted ~ productivity + gender + leadership_for_level, data = testing, family = binomial(link = "logit"))
summary(model_2)
confint(model_2)

# add random intercept for teams 
model_3 <- gamm4(promoted ~ productivity + gender+ leadership_for_level+ s(team, bs = "re") , family = binomial(link = "logit"), data = testing)

# model with only random effect
model_4 <- glmer(promoted ~ 1 + (1|team), family = binomial , data = testing)
summary(model_4)
confint(model_4)
```
