...
The "current employee dataset" consisted of information on the current employees at Black Saber Software. It recorded each employee's quarterly data on gender, leadership level, their role-seniority within the organization, productivity, the team they belonged to, and salary they received in each quarter. The employee information was recorded anonymously so that the privacy of the employee is safely protected in our assessment.
...
We take an initial look at the number of males and females who applied to each team and how far along the hiring process they reached. Figure XX shows us that roughly the same number of men and women made it to phase 2. In fact, there were 89 women in this phase who applied to the Software team, compared to only 78 men. However, figure YY shows that roughly double the number of men were able to move on to phase 3 in both teams. This immediately draws attention because the pattern happened in both teams. Finally, 4 men and 1 woman were hired in both teams, as seen in figure ZZ - once again, for both teams. So overall, roughly the same number of men and women applied for the new grad position, but 4 times more men were hired.
...
Since the hiring decision is based on factors ranging from AI grades to the interviews and the AI is biased, we can conclude that the hiring process at Black Saber is biased towards men. On the bright side, the issues reside in the AI; the actual employees are not biased, which would have been a more difficult problem to handle.
